...
\begin{quotation}
    ...
    \fuente{\url{https://www.linuxadictos.com/disponible-la-nueva-version-de-alpine-linux-3-8-1.html#google_vignette}}
\end{quotation}
...
\begin{itemize}
    ...
    \item Configurar el proxy (si lo hay) con el comando \$ setup-proxy y luego importarlo en las variables de entorno locales\$ source /etc/profile.d/proxy.sh
    \item Ejecutar \$ setup-apkcache para crear el cache local de paquetes de Alpine Linux teniendo en cuenta que, en general, el cache quedará en / media / mmcblk0p2 / cache
    \item Sincronizar el caché local con los repositorios mediante el comando \$ apk cache -v sync.
\end{itemize}
    
Los archivos .apkovl.tar.gz son archivos de {backup local de configuración}  que se usan para hacer mas reproducible la configuración de los gateways.

\subsection{LoRa packet forwarder}

Es el software que se coloca en {host} el {gateway LoRa}, el cual reenvía los paquetes de RF que recibe del concentrador a un servidor a través de un enlace IP / UDP.
