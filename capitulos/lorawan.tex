\chapter{Fundamentos de LoRaWAN}

LoRaWAN es una \gls{wan} descrita en un modelo de tres capas: la de aplicación, la de control de acceso al medio (\gls{mac}) y la física o de transmisión, según se muestra en la figura \ref{f:m_protocolos}.
...
\begin{figure}[H]
  \centering
  \caption{Tiempos de comunicación nodo-pasarela-nodo}\label{f:retardo}
  \begin{tikzpicture}
...
Los nodos y la pasarela tienen comunicación bidireccional con ranuras de recepción programadas. Los nodos de clase B, además, tienen las funcionalidades de la clase A. Para alcanzar la sincronización de la red, los nodos reciben un tiempo de sincronización tipo \emph{beacon} desde la pasarela, lo que permite al servidor saber cuándo un nodo está escuchando. Los nodos que hablen en la ventana de tiempo programada entran a la pasarela de acuerdo a procesos aleatorios con prioridad en servicios a los nodos en mención.
...
Las tramas de la capa física se distinguen entre las de transmisión (\emph{uplink}) y las de recepción (\emph{downlink}), desde el punto de vista del nodo.

El nodo transmite la trama a la pasarela (figura~\ref{f:retardo1}), y de ahí al servidor de red. La trama de la capa física de LoRa está compuesta por el preámbulo, el \gls{phdr}, el \gls{pc}, la carga útil (\emph{payload}) y el \gls{crc}.

\begin{figure}[H]
  \caption{Estructura de tramas de \emph{uplink} y \emph{downlink}}
  \label{f:retardo1}
  \centering
...
\begin{figure}[H]
  \caption{Formato del mensaje LoRa}\label{f:m_lora}
  \centering
  \includegraphics[width =.8\textwidth]{capitulos/figuras/f2.tikz}

  \propia
\end{figure}

En la \gls{pp}, se encuentra el contenido de los diferentes tipos de tramas \gls{mac}, las cuales tienen en común el encabezado \gls{mhdr} y el \gls{mic}. Los tipos de tramas \gls{mac} corresponden a los de la \gls{mp}, solicitud de acceso (\emph{join-request}) y respuesta a la solicitud (\emph{join-response}).

La trama \gls{mp} contiene la información de los nodos. Los campos de esta trama corresponden a la \gls{fhdr}, al \gls{fp} y a la \gls{fpa}. La \gls{fhdr} contiene los campos de \gls{da}, \gls{fc}, \gls{fcn} y las \gls{fo}.

\section{Características de LoRaWAN}

\textbf{Banda de frecuencias y canalización}. La banda EU-868 del estándar \mbox{LoRa} tiene un rango de \SI{863}{\MHz} a \SI{870}{\MHz} y también está definida en el espectro de radio para la banda \gls{ism} normatizada por \gls{etsi}, en el estándar EN300.220. Los canales de la red pueden ser atribuidos libremente por el operador. La pasarela posee ocho canales en paralelo, del IF0 al IF7, como medio de comunicación con el nodo. El ancho de banda es fijo, 125~kHz, con tasa de bit variable de acuerdo al \gls{sf}.

\begin{figure}[H]
  \centering
  \caption{Canalización de la banda ISM868}
  \label{f:m_canales}
  \includegraphics[width =\textwidth]{capitulos/figuras/canales}
  \propia
\end{figure}

Además, tiene dos canales adicionales: uno LoRa, con ancho de banda fijo de \SI{250}{\kHz}, con una tasa de bit de 11~kbps, como el canal \emph{back-haul} de la red LoRaWAN IF1, y otro canal de \gls{gfsk}, el cual tiene una tasa de datos y un ancho de banda ajustables para una velocidad máxima de 50 kbit/s \cite{LoRa-2015}.

\textbf{Factor de ensanchado (\gls{sf}), tasa de bit y potencia}. Permite un manejo eficiente del espectro y de la capacidad de la red LoRa. Hay seis \gls{sf}, que van del SF7 al SF12, los cuales están relacionados con la potencia y la tasa de datos. La tasa de datos va desde 250~bps, para \gls{sf}12, con una sensibilidad de recepción de -137~dBm, hasta 5.5~kbps, para \gls{sf}7, con una sensibilidad de -123~dBm. La potencia máxima de transmisión está, de acuerdo a \gls{etsi}, en 14~dBm. LoRa utiliza el ajuste dinámico de la potencia de emisión y la \gls{adr} de acuerdo a la distancia que separa el nodo de la pasarela, y de esta manera se maneja la eficiencia de la energía.

\section{Seguridad en LoRaWAN}
...
Los mensajes de aplicación de la capa \gls{mac} de LoRaWAN tienen autenticación de origen, integridad protegida, reproducción protegida y encriptación. Lo anterior, unido a la autenticación mutua, garantiza que el tráfico en la red no sea alterado y que provenga de un nodo legítimo de la red. Esto hace que la información no sea comprensible para los intrusos y tampoco capturada o alterada por los piratas. La tecnología LoRa es una de las pocas redes IoT que implementa encriptación de extremo a extremo. En las redes celulares, el tráfico es encriptado en la interfaz de aire, pero es transportado como texto plano en el núcleo de la red del operador, lo cual puede ser vulnerado de forma selectiva. Para este problema se creó una etapa adicional para la seguridad. LoRaWAN no provee esta capa adicional, ya que se aumentarían el consumo de potencia, la complejidad y los costos.

Potentes algoritmos de seguridad están implementados en la red LoRa, como lo es el \gls{aes}, el cual ha sido adoptado para la seguridad del tráfico entre los nodos y la red en conjunto con los diferentes modos de operación, como \gls{cmac} para proteger la integridad de la información, y el \gls{crt}, para la encriptación. Los nodos son personalizados con una clave \gls{aes} de 128~bit (AppKey) y un único identificador global (EUI-64-based DevEUI), como se observa en la figura~\ref{f:seg1}.

\begin{figure}[H]
  \centering
  \caption{Seguridad en la trama LoRaWAN}
  \label{f:seg1}
  \includegraphics[width =.85\textwidth]{capitulos/figuras/seguridad1}
  \propia
\end{figure}

La activación al aire (\emph{over-the-air}) provee el reconocimiento de ambos, el nodo y la red. Por medio de la llave \emph{AppKey}, el nodo se puede unir a la red (figura \ref{f:seg2}).
Hay dos llaves de sesión: \emph{NwkSKey}, utilizada para la integridad y autenticación del tráfico de la capa \gls{mac} de \gls{lwan}, y \emph{AppSKey}, utilizada para la encriptación de extremo a extremo de la carga útil de la aplicación.

\begin{figure}[H]
  \centering
  \caption{Seguridad en LoRaWAN}
  \label{f:seg2}
  \includegraphics[width =.85\textwidth]{capitulos/figuras/seguridad2}
  \propia
\end{figure}

La llave NwkSKey es distribuida por la red LoRaWAN en su orden para probar y verificar la autenticidad y la integridad de los paquetes. La llave AppSKey la provee el servidor de la aplicación para encriptar y desencriptar la carga útil de la aplicación. Las llaves AppSKey y AppKey están ocultas al operador de la red, razón por la cual no se puede desencriptar la información del usuario.
...
La combinación de \gls{iot} y \gls{wsn} permite un desarrollo revolucionario de los sistemas de comunicación con mejoras en la confiabilidad, la flexibilidad, el cubrimiento y la eficiencia.
...
Las aplicaciones de la tecnología \gls{wsn} permiten el desarrollo de sistemas inteligentes en áreas como manejo de aguas, transporte, hogar, industria, entre otras, y siempre con mejoras importantes sobre las tecnologías tradicionales.

Agregado a lo anterior, el mundo moderno de las \gls{wsn} emigra hacia una nueva era con su integración al \gls{iot}, donde habrá una gran variedad de temas que deberán ser normados y aceptados en un corto tiempo. Uno de los temas más apremiantes es la propiedad, uso y legalidad de los datos que se generan, directa o indirectamente, gracias a dichos sistemas.

En su estructura fundamental, una \gls{wsn} está constituida por:
\begin{itemize}
  \item Los nodos sensores dispuestos para la detección de las diferentes magnitudes y para la comunicación.
  ...
  \item La pasarela o \emph{gateway} encargada de la conexión a internet.
  \item Los clientes.
\end{itemize}

Los nodos transmiten y reciben el estado de cada uno de ellos entre sí para reconocerse como existentes en la red. Los nodos configuran una red (estrella, malla, o cualquier otra) con cubrimiento de distancias cortas con un máximo de 1000~metros y utilizando la técnica multisalto.
